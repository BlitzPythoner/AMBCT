...
def welcome():
    os.system("cls")
    print(f"=== Automatic Manual Backup Creation Tool {__version__} {__beta__} ===\n")
    print("Welcome to AMBCT!\nChoose an action:\n")
    print("[1] Create a new backup")
    print("[2] Append an existing backup")
    print("[3] Image Configuator\n")
    print("[0] Need help?")
    if update == 1:
    ...
    if update == 3:
        print(f"AMBCT {__version__} is in development")
        print("**NOTE** This is a Beta, not all functions will work correctly!")
    while True:
        choice = input("\nChoose a path: ").lower()
...
def image_configurator():
    if "wim_path" not in locals():
        wim_path = ""
    os.system("cls")
    ...
    def delete_backup():
        ...
        try:
            ...
            input(f"Error while calling indexes, Error: {e}\nPress any key to get back to main menu...")
            return 
        ...
        try:
            ...
            input(f"Deleting of index {sel_idx} in {wim_path} failed. Error: {e}\nPress any key...")
            return
    ...
    def check_backup():
        ...
        try:
            ...
            input(f"Verifying of {wim_path} failed. Error: {e}\nPress any key...")
            return
